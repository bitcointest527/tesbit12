...
###   HOST1 and HOST2 and compares them.
...
WORKINGDIR="/tmp/bitcoinphantom_verify_binaries"
TMPFILE="hashes.tmp"
...
HOST2=""
BASEDIR="/bin/"
VERSIONPREFIX="bitcoinphantom-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinphantom-',
...
if [ $? -ne 0 ]; then
   echo "HOST1 failed to provide signature file, but HOST2 did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "HOST1 and HOST2 signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the bitcoinphantom Core binary release signing key installed?"
   fi
