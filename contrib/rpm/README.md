...
The RPM spec file provided here is for bitcoinphantom-Core 0.12.0 and builds on CentOS
...
necessary, the packager should bring the problem to the attention of the bitcoinphantom
...
needed when 0.12.1 is released, a proper fix is already in the bitcoinphantom git
master branch.
...
idea to build bitcoinphantom Core against a static version of that library compiled
...
The `bitcoinphantom.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bitcoinphantom-server` package. At this point, most RPM based
...
The `bitcoinphantom-server` package should have SELinux support. How to properly do
...
bitcoinphantom-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bitcoinphantom/). He has been
packaging bitcoinphantom for Fedora at least since 2012.
...
4. I renamed the `bitcoinphantom` package that contains the Qt GUI to `bitcoinphantom-core` as
that appears to be how the general population refers to it, in contrast to
`bitcoinphantom-xt` or `bitcoinphantom-classic`. I wanted to make sure the general population
