...
#// full deployement :   wget -O - https://raw.githubusercontent.com/badbrainIRC/bitcoinphantom/master/go.sh | bash
sudo rm -Rf ~/bitcoinphantom
...
git clone --recurse-submodules https://github.com/bitcoinphantom/bitcoinphantom
cd ~
cd bitcoinphantom
./autogen.sh
chmod 777 ~/bitcoinphantom/share/genbuild.sh
chmod 777 ~/bitcoinphantom/src/leveldb/build_detect_platform
...
sed -i 's/#include <emmintrin.h>//g' ~/bitcoinphantom/src/crypto/pow/scrypt-sse2.cpp
else
txt=$(echo "")
fi
...
if [ -e ~/bitcoinphantom/src/qt/bitcoinphantom-qt ]
then
sudo strip ~/bitcoinphantom/src/bitcoinphantomd
sudo strip ~/bitcoinphantom/src/qt/bitcoinphantom-qt
sudo make install
else
echo "Compile fail not bitcoinphantom-qt present"
fi
...
mkdir -p ~/.bitcoinphantom
if [ -e ~/.bitcoinphantom/bitcoinphantom.conf ]
then
    cp -a ~/.bitcoinphantom/bitcoinphantom.conf ~/.bitcoinphantom/bitcoinphantom.bak
fi
rm ~/.bitcoinphantom/bitcoinphantom.conf
echo "rpcuser="$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 26) >> ~/.bitcoinphantom/bitcoinphantom.conf
echo "rpcpassword="$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 26) >> ~/.bitcoinphantom/bitcoinphantom.conf
echo "rpcport=11207" >> ~/.bitcoinphantom/bitcoinphantom.conf
echo "port=11211" >> ~/.bitcoinphantom/bitcoinphantom.conf
echo "daemon=1" >> ~/.bitcoinphantom/bitcoinphantom.conf
echo "listen=1" >> ~/.bitcoinphantom/bitcoinphantom.conf
echo "server=1" >> ~/.bitcoinphantom/bitcoinphantom.conf
echo "deprecatedrpc=accounts" >> ~/.bitcoinphantom/bitcoinphantom.conf

echo "usehd=1" >> ~/.bitcoinphantom/bitcoinphantom.conf
...
mkdir -p ~/Desktop/
sudo cp ~/bitcoinphantom/src/qt/res/icons/bitcoinphantom.png /usr/share/icons/
echo "#!/usr/bin/env xdg-open" >> ~/Desktop/bitcoinphantom.desktop
echo "[Desktop Entry]" >> ~/Desktop/bitcoinphantom.desktop
echo "Version=1.0" >> ~/Desktop/bitcoinphantom.desktop
echo "Type=Application" >> ~/Desktop/bitcoinphantom.desktop
echo "Terminal=false" >> ~/Desktop/bitcoinphantom.desktop
echo "Icon[en]=/usr/share/icons/bitcoinphantom.png" >> ~/Desktop/bitcoinphantom.desktop
echo "Name[en]=bitcoinphantom" >> ~/Desktop/bitcoinphantom.desktop
echo "Exec=bitcoinphantom-qt" >> ~/Desktop/bitcoinphantom.desktop
echo "Name=bitcoinphantom" >> ~/Desktop/bitcoinphantom.desktop
echo "Icon=/usr/share/icons/bitcoinphantom.png" >> ~/Desktop/bitcoinphantom.desktop
echo "Categories=Network;Internet;" >> ~/Desktop/bitcoinphantom.desktop
sudo chmod +x ~/Desktop/bitcoinphantom.desktop
sudo cp ~/Desktop/bitcoinphantom.desktop /usr/share/applications/bitcoinphantom.desktop
sudo chmod +x /usr/share/applications/bitcoinphantom.desktop

# Erase all bitcoinphantom compilation directory , cleaning
...
echo -n "Success....Blockchain is now downloading press Ctrl-C to cancel but it will take longer to sync from 0. And you will have to start bitcoinphantom manual"
sudo rm QT-Wallet*.zip
echo "wget --no-check-certificate " $(lynx --dump --listonly https://bitcoinphantom-blockchain.com/down/ | grep -o "https://bitcoinphantom-blockchain.com/blockchain5.*zip" | head -1 ) > link.sh
sh link.sh
unzip -o QT-Wallet*.zip -d ~/.bitcoinphantom
sudo rm QT-Wallet*.zip
#// Start bitcoinphantom
bitcoinphantom-qt
