...
/*BOOST_AUTO_TEST_CASE(bloom_create_insert_serialize)
{
    ...
    filter.insert(ParseHex("d02b63bbd127a69b99fb97ff7d6eb01e586bd157"));
    BOOST_CHECK_MESSAGE( filter.contains(ParseHex("743053ad2197d82f320fdb06c33a4bd1f26a5566")), "Bloom filter doesn't contain just-inserted object!");
    // One bit different in first byte
    BOOST_CHECK_MESSAGE(!filter.contains(ParseHex("cfb2f1bdc62d5a697558ebb3c211cfd411c4d446")), "Bloom filter contains something it shouldn't!");

    filter.insert(ParseHex("c8630f6b4b54af570fbbf2604fd8bb8e6cb45e0b"));
    BOOST_CHECK_MESSAGE(filter.contains(ParseHex("c8630f6b4b54af570fbbf2604fd8bb8e6cb45e0b")), "Bloom filter doesn't contain just-inserted object (2)!");

    filter.insert(ParseHex("f83b5b37096740857ff14614594a863c21b6c971"));
    BOOST_CHECK_MESSAGE(filter.contains(ParseHex("f83b5b37096740857ff14614594a863c21b6c971")), "Bloom filter doesn't contain just-inserted object (3)!");

    CDataStream stream(SER_NETWORK, PROTOCOL_VERSION);
    stream << filter;

    std::vector<unsigned char> vch = ParseHex("a085707c339a41c9a4b8e06657");
    ...
    BOOST_CHECK_MESSAGE( filter.contains(ParseHex("67ebb60a336b8c970ab1254cb5cae2c6da97b8e3")), "Bloom filter doesn't contain just-inserted object!");
    filter.clear();
    BOOST_CHECK_MESSAGE( !filter.contains(ParseHex("67ebb60a336b8c970ab1254cb5cae2c6da97b8e3")), "Bloom filter should be empty!");
}
...
BOOST_AUTO_TEST_CASE(bloom_create_insert_serialize_with_tweak)
{
    ...
    filter.insert(ParseHex("c240c5ffc8410b1997ef2935208a25737d0e5a00"));
    BOOST_CHECK_MESSAGE( filter.contains(ParseHex("67ebb60a336b8c970ab1254cb5cae2c6da97b8e3")), "Bloom filter doesn't contain just-inserted object!");
    // One bit different in first byte
    BOOST_CHECK_MESSAGE(!filter.contains(ParseHex("67ebb60a336b8c970ab1254cb5cae2c6da97b8e3")), "Bloom filter contains something it shouldn't!");
    ...
    filter.insert(ParseHex("db26be306fcaf98c7e5d5c9065af4dc8ebb90e09"));
    BOOST_CHECK_MESSAGE(filter.contains(ParseHex("db26be306fcaf98c7e5d5c9065af4dc8ebb90e09")), "Bloom filter doesn't contain just-inserted object (3)!");

    CDataStream stream(SER_NETWORK, PROTOCOL_VERSION);
    stream << filter;

    std::vector<unsigned char> vch = ParseHex("3cdf57260f1e06c72cd80771d4");
    std::vector<char> expected(vch.size());
...
BOOST_AUTO_TEST_CASE(bloom_create_insert_key)
{
    ...
    std::vector<unsigned char> vch = ParseHex("e577d6e2d10bce85525f77294f");
    std::vector<char> expected(vch.size());
...
BOOST_AUTO_TEST_CASE(rolling_bloom)
{
    ...
    // Run test_bitcoinphantom with --log_level=message to see BOOST_TEST_MESSAGEs:
