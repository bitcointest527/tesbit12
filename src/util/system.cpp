...
const char * const bitcoinphantom_CONF_FILENAME = "bitcoinphantom.conf";
const char * const bitcoinphantom_PID_FILENAME = "bitcoinphantomd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bitcoinphantomd -foo=bar
...
#else
    const char* pszModule = "bitcoinphantom";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\bitcoinphantom
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\bitcoinphantom
    // Mac: ~/Library/Application Support/bitcoinphantom
    // Unix: ~/.bitcoinphantom
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "bitcoinphantom";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/bitcoinphantom";
#else
    // Unix
    return pathRet / ".bitcoinphantom";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", bitcoinphantom_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", bitcoinphantom_PID_FILENAME)));
}
