...
#if defined(NDEBUG)
# error "bitcoinphantom cannot be compiled without assertions."
#endif
...
public:
    ...
    bool LoadBlockIndex(const CChainParams& chainparams, const Consensus::Params& consensus_params, CBlockTreeDB& blocktree);
...
const std::string strMessageMagic = "bitcoinphantom Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s btcx additional fees, %d delta bytes\n",
...
static void CheckForkWarningConditions()
{
    ...
    // If our best fork is no longer within 72 blocks (+/- 110 seconds if no one mines it)
    // of our head, drop it
...
static void CheckForkWarningConditionsOnNewFork(CBlockIndex* pindexNewForkTip)
{
    ...
    // with a tip within 72 blocks (+/- 110 seconds if no one mines it) of ours
...
void ThreadScriptCheck() {
    RenameThread("bitcoinphantom-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Also, currently the rule against blocks more than 10 seconds in the future
    ...
    if(pindex->nHeight < chainparams.GetConsensus().ForkHeight)
        fScriptChecks = false; // FIXME: testing
    ...
    // We don't have a hash as it is in the future
...
bool CChainState::LoadBlockIndex(const CChainParams& chainparams, const Consensus::Params& consensus_params, CBlockTreeDB& blocktree)
{
    ...
    for (const std::pair<int, CBlockIndex*>& item : vSortedByHeight)
    {
        ...
        // Check the block index checkpoints to verify
        // the correctness/validity of the blockchain
...
{
    if (!g_chainstate.LoadBlockIndex(chainparams, chainparams.GetConsensus(), *pblocktree))
        return false;
