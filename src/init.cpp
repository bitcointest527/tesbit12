...
// Copyright (c) 2019-2019 bitcoinphantom Core developers
 ...
 * between bitcoinphantomd, and bitcoinphantom-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("bitcoinphantom-shutoff");
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", bitcoinphantom_CONF_FILENAME), false, OptionsCategory::OPTIONS);
...
#ifndef WIN32
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", bitcoinphantom_PID_FILENAME), false, OptionsCategory::OPTIONS);
#else
    ...
    gArgs.AddArg("-algo=<algorithm>", "Sets algorithm for mining, can be changed with cli/rpc later (default: scrypt)", false, OptionsCategory::OPTIONS);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/bitcoinphantom/bitcoinphantom>";
    const std::string URL_WEBSITE = "<https://bitcoinphantom.com>";

    return CopyrightHolders(strprintf(_("Copyright (C) %i-%i"), 2009, COPYRIGHT_YEAR) + " ") + "\n" +
           strprintf(_("Copyright (C) %i-%i The bitcoinphantom Core developers"), 2019, COPYRIGHT_YEAR) + "\n" +
           "\n" +
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("bitcoinphantom-loadblk");
...
/** Sanity checks
 *  Ensure that bitcoinphantom is running in a usable environment with all
...
{
    // Make sure only a single bitcoinphantom process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", bitcoinphantom_CONF_FILENAME)).string());
    LogPrintf("Using at most %i automatic connections (%i file descriptors available)\n", nMaxConnections, nFD);

    // Warn about relative -datadir path.
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if bitcoinphantom is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if bitcoinphantom is started while in a temporary directory.\n",
    ...
    if (!gArgs.IsArgSet("-without-tor")) {
        ...
        if (!Lookup(defaultPort.c_str(), onionProxy, DEFAULT_TOR_PORT, false/*name lookup*/))
            return InitError(strprintf(_("Invalid -onion address or hostname: 127.0.0.1:9089")));
        

        proxyType addrOnion = proxyType(onionProxy, true);
