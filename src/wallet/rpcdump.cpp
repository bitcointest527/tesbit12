...
UniValue importprivkey(const JSONRPCRequest& request)
{
    ...
    bool fRescan = true;
    {
        ...
        CKeyID vchAddress = pubkey.GetID();
        {
            ...
            if (pwallet->HaveKey(vchAddress)) {
                result.pushKV("message", "Privake key has already been imported");
    ...
    result.pushKV("message", "Private key has been successfully imported");
    return JSONRPCReplyObj(result, NullUniValue, 1);
    //return "Private key has been successfully imported";
}
...
UniValue importaddress(const JSONRPCRequest& request)
{
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid bitcoinphantom address or script");
        }
    }
    if (fRescan)
    {
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The bitcoinphantom address for the private key\n"
            "\nResult:\n"
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename with path (either absolute or relative to bitcoinphantomd)\n"
            "\nResult:\n"
            "{                           (json object)\n"
    ...
    // produce output
    file << strprintf("# Wallet dump created by bitcoinphantom %s\n", CLIENT_BUILD);
...
UniValue importmulti(const JSONRPCRequest& mainRequest)
{
    ...
    if (mainRequest.fHelp || mainRequest.params.size() < 1 || mainRequest.params.size() > 2)
        throw std::runtime_error(
            ...
            "                                                              0 can be specified to scan the entire blockchain. Blocks up to 10 seconds before the earliest key\n"
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see bitcoinphantomd log for details) and could "
