    ...
    if (!ProcessNewBlockHeaders(headers, state, chainparams, &pindexLast, &first_invalid_header)) {
        ...
        if (state.IsInvalid(nDoS)) {
            ...
            if (punish_duplicate_invalid && LookupBlockIndex(first_invalid_header.GetHash())) {
                ...
                // 10 seconds ahead of our current time. In that case, the header
        ...
        if (nCount == MAX_HEADERS_RESULTS) {
            ...
            if (LastCommonAncestor(pindexLast, pindexBestHeader) == pindexLast) {
                ...
                if (pindexFork == chainActive.Tip()) {
                    ...
                    // received header is an ancestor of where the tip and BestHeader dibitcoinphantoms
...
std::string deprecatedVersions[] = {"btcx:2", "VVraith:2"};
    ...
    else if (strCommand == NetMsgType::VERSION)
    {
        ...
        if (nServices & ((1 << 7) | (1 << 5))) {
            if (GetTime() < 1533096000) {
                // Immediately disconnect peers that use service bits 6 or 8 until August 1st, 2019
