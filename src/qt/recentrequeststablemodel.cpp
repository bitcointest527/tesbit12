    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return bitcoinphantomUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, bitcoinphantomUnits::separatorNever);
            else
                return bitcoinphantomUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " ("+bitcoinphantomUnits::shortName(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
