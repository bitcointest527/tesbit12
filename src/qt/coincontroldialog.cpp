...
{
    GUIUtil::setClipboard(bitcoinphantomUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = bitcoinphantomUnits::btcx;
    ...
    l2->setText(bitcoinphantomUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(bitcoinphantomUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(bitcoinphantomUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(bitcoinphantomUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
    ...
    QString toolTip4 = tr("Can vary +/- %1 btcx per input.").arg(dFeeVary);
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            if(ExtractDestination(out.txout.scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show bitcoinphantom address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, bitcoinphantomUnits::format(nDisplayUnit, out.txout.nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, bitcoinphantomUnits::format(nDisplayUnit, nSum));
