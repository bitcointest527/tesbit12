...
import RPC call and sends XSH to it. Then other nodes import the addresses,
and the test makes listtransactions and getbalance calls to confirm that the
importing node either did or did not execute rescans picking up the send
transactions.

In the second part of the test, node 0 sends more XSH to each address, and the
...
# Rescans start at the earliest block up to 10 seconds before the key timestamp.
TIMESTAMP_WINDOW = 2 * 60 * 60


class ImportRescanTest(bitcoinphantomTestFramework):
