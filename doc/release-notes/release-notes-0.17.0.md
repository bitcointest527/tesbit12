...
On Mac, copy over `/Applications/bitcoinphantom-Qt` 
or on Linux `bitcoinphantomd`/`bitcoinphantom-qt`.
...
bitcoinphantom is extensively tested on multiple operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer (Windows XP is not supported).

bitcoinphantom should also work on most other Unix-like systems but is not
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `bitcoinphantom.conf` file, not inside included files or from
  ...
  as bitcoinphantom.conf will still include `relative.conf`.
...
prefixing the option with the network, such as:
    ...
    main.uacomment=bitcoinphantom
    test.uacomment=bitcoinphantom-testnet
...
be used in V0.17 by starting bitcoinphantomd with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed bitcoinphantom Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for bitcoinphantom transactions that are not fully signed
...
Overall, the construction of a fully signed bitcoinphantom transaction goes through the
following steps:
...
- An **Extractor** produces a valid bitcoinphantom transaction (in network format)
...
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When bitcoinphantom is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If bitcoinphantom is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart bitcoinphantomd with
...
- The log timestamp format is now ISO 8601 (e.g. "2019-02-28T12:34:56Z").

- When running bitcoinphantomd with `-debug` but without `-daemon`, logging to stdout
...
- #12402 `59e032b` expat 2.2.5, ccache 3.4.1, miniupnpc 2.0.20190203 (fanquake)
...
- #13604 `75bea05` Add depends 32-bit arm support for bitcoinphantom-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling bitcoinphantom-qt (TheCharlatan)
...
- #13479 `fa2ea37` contrib: Fix cve-2019-12356 by hardening the regex (loganaden)
...
- awemany (for CVE-2019-17144, previously credited as "anonymous reporter")
