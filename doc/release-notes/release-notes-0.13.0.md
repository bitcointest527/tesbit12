...
The two phases are distinct in the bitcoinphantom-qt GUI. During the first one,
...
- `gettxoutsetinfo` UTXO hash (`hash_serialized`) has changed. There was a dibitcoinphantomnce between
...
- #7848 `88616d2` Dibitcoinphantomnce between 32- and 64-bit when hashing >4GB affects `gettxoutsetinfo` (laanwj)
...
- #7613 `3798e5d` Add autocomplete to bitcoinphantom-qt's console window (GamerSg)
...
- #8256 `af2421c` BUG: bitcoinphantom-qt crash (fsb4000)
