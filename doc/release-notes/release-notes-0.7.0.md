...
We recommend that everybody running prior versions of bitcoind/bitcoinphantom-qt
...
in your terminal, then install the bitcoinphantom-qt package:
  sudo apt-get install bitcoinphantom-qt
...
Code:
/Applications/bitcoinphantom-qt
(on Mac) or
Code:
bitcoind/bitcoinphantom-qt
(on Linux).
...
* (Windows only): enable ASLR and DEP for bitcoinphantom-qt.exe
* (Windows only): add meta-data to bitcoinphantom-qt.exe (e.g. description)
