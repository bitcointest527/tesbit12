...
*Setup instructions for a Gitian build of bitcoinphantom Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the bitcoinphantom
...
- [Building bitcoinphantom Core](#building-bitcoinphantom-core)
...
```bash
cp bitcoinphantom/contrib/gitian-build.py .
```
...
```
git clone git@github.com:bitcoinphantom/gitian.sigs.git
...
```
export NAME=satoshi
gpg --output $VERSION-linux/$NAME/bitcoinphantom-linux-0.16-build.assert.sig --detach-sign 0.16.0rc1-linux/$NAME/bitcoinphantom-linux-0.16-build.assert 
gpg --output $VERSION-osx-unsigned/$NAME/bitcoinphantom-osx-0.16-build.assert.sig --detach-sign 0.16.0rc1-osx-unsigned/$NAME/bitcoinphantom-osx-0.16-build.assert 
gpg --output $VERSION-win-unsigned/$NAME/bitcoinphantom-win-0.16-build.assert.sig --detach-sign 0.16.0rc1-win-unsigned/$NAME/bitcoinphantom-win-0.16-build.assert 
```

Make a PR (both the `.assert` and `.assert.sig` files) to the
[bitcoinphantom-core/gitian.sigs](https://github.com/bitcoinphantom/gitian.sigs/) repository:
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitcoinphantom-linux-*-build.assert
    gpg --detach-sign ${VERSION}-win-unsigned/${SIGNER}/bitcoinphantom-win-*-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bitcoinphantom-osx-*-build.assert
```
